...
**Версия 3.4.4**  
<span style="color:green">_Примечание: Конвертирован из word_</span>
...
<span style="color:red">Раз в 10 минут, либо перед каждой отправкой заявки необходимо запрашивать актуальный для работы token.</span>

<span style="color:green">_Примечание: на текущий момент token всегда один и тот же_</span>
...
```
<span style="color:red">Полученное в элементе `body/token` требуется использовать для дальнейшей работы с API.</span>
...
<span style="color:red">Передача `json` заказа должна осуществляться в UTF-8. Все поля должны существовать, даже если не заполнены.</span>
...
<span style="color:green">_Примечание: нахера делать `json` текстом, сие есть великая тайна_</span>
...
В примере используется тестовый параметр depart\_number и призван показать принцип формирования пакетных запросов.
```java
...
<span style="color:green">_Примечание: нифига ответ другой (походу это уже старая версия), к тому же есть комент разрабов:_</span>
...
```
<span style="color:red">Примеры реализации запросов на curl, Delphi, php, 1C могут быть предоставлены по запросу</span>
...
<span style="color:red">Все элементы должны присутствовать. Даже если каких-то данных нет, наличие параметра обязательно.</span>
...
<span style="color:red">Для корректности обработки результатов требуется каждую услугу отправлять отдельным запросом.</span>
