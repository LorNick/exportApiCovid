...
| **Версия** | **Дата изм-ния** | **Описание изменения** | **Автор изменения**&nbsp;&nbsp;&nbsp;&nbsp; |
| --- | :---: | --- | ---- |
| 1.4.9 | 04.12.20 | Изменены требования к элементу `serv`: теперь для отправки нескольких услуг по одному заказу требуется его "разбивать" на несколько отдельных заказов. [Раздел элемент `order/serv`](#элемент-orderserv) | Евстифеев Е.А. |
| 2.0.0 | 05.12.20 | Добавлены примеры построения curl-запросов и ответов на запросы. [Раздел пример запроса в curl](#пример-запроса-в-curl) | Евстифеев Е.А. |
| 2.0.0 | 05.12.20 | Добавлено описание методов получения статуса заявок. Описано в разделе [Получение статуса обработки заявки](#получение-статуса-обработки-заявки) | Евстифеев Е.А. |
| 2.0.0 | 05.12.20 | Добавлена пакетная отправка заявок. Описано в разделе [Получение статуса. Пакетное получение статусов](#получение-статуса-пакетное-получение-статусов). | Евстифеев Е.А. |
| 2.0.0 | 05.12.20 | Обновлены этапы тестирования работы с методами. Описано в разделе [Этапы тестирования отправки заявки](#этапы-тестирования-отправки-заявки) | Евстифеев Е.А. |
| 2.0.0 | 05.12.20 | Добавлен раздел с часто задаваемыми вопросами. Описано в разделе [Часто задаваемые вопросы](#часто-задаваемые-вопросы) | Евстифеев Е.А. |
| 2.0.1 | 07.12.20 | Обновлены варианты ответа в разделе [Получение статуса. Количество не полученных статусов](#получение-статуса-количество-не-полученных-статусов) | Евстифеев Е.А. |
| 2.0.2 | 07.12.20 | В раздел [Часто задаваемые вопросы](#часто-задаваемые-вопросы) добавлены новые вопросы, связанные с ФИО гражданина РФ и нерезидентами РФ. | Евстифеев Е.А. |
| 2.0.3 | 07.12.20 | В раздел [Элемент `order/serv`](#элемент-orderserv) добавлен вариант результата: сомнительно. | Евстифеев Е.А. |
| 2.1.1 | 01.03.21 | Добавлены дополнительные значения поля `result` в раздел [Элемент `order/serv`](#элемент-orderserv) | Евстифеев Е.А. |
| 3.1.1 | 02.04.21 | Обновлено описание всех основных разделов | Евстифеев Е.А. |
| 3.2.1 | 05.04.21 | Расширено описание раздела [Отправка других типов исследований](#отправка-других-типов-исследований) | Евстифеев Е.А. |
...
| 3.4.4 | 16.06.21 | В `orderDate` добавлена ограничение -6 месяцев +10 дней | Самсонов А.А. |
| 3.4.4 | 24.06.21 | Изменение кейсов для [этапов тестирования отправки заявки](#этапы-тестирования-отправки-заявки) | Аландаров М.А. |
...
- _**[Отправка результата по COVID](#отправка-результата-по-covid)**_
  ...
  - **[Передача пакета заявок](#передача-пакета-заявок)**
    ...
    - [Пример неуспешного ответа (неверный токен)](#пример-неуспешного-ответа-неверный-токен)
    - [Пример неуспешного ответа (неверный ЛПУ)](#пример-неуспешного-ответа-неверный-лпу)
    - [Пример тело запроса](#пример-тело-запроса)
  - **[Отправка других типов исследований](#отправка-других-типов-исследований)**
  - **[Подробное описание параметра `json`](#подробное-описание-параметра-json)**
    - [Элемент `order`](#элемент-order)
    - [Элемент `order/serv`](#элемент-orderserv)
    - [Элемент `order/patient`](#элемент-orderpatient)
    - [Элемент `order/address/regAddress`](#элемент-orderaddressregaddress)
...
- _**[Получение статуса обработки заявки](#получение-статуса-обработки-заявки)**_
  - **[Получение статуса. Количество не полученных статусов](#получение-статуса-количество-не-полученных-статусов)**
  - **[Получение статуса. Пакетное получение статусов](#получение-статуса-пакетное-получение-статусов)**
  - **[Получение статуса. Получение статусов конкретных заявок](#получение-статуса-получение-статусов-конкретных-заявок)**
- _**[Этапы тестирования отправки заявки](#этапы-тестирования-отправки-заявки)**_
- _**[Часто задаваемые вопросы](#часто-задаваемые-вопросы)**_
- _**[Информация по автоматической выгрузке ПЦР-тестов на коронавирусную инфекцию на портал Госуслуг (физлица)](#информация-по-автоматической-выгрузке-пцр-тестов-на-коронавирусную-инфекцию-на-портал-госуслуг-физлица)**_
...
Дополнительная информация: [Часто задаваемые вопросы](#часто-задаваемые-вопросы)  
...
| --- | --- | :---: | --- |
| `number` | varchar(30) | ДА | Номер заказа. Используется для идентификации запроса. Номер запроса должен быть уникальным. Даже в рамках тестирования нужно обеспечить уникальность номера заказа. Для тестирования можно использовать нумерацию: TEST-001. |
...
| `type` | int | НЕТ | Значение в соответствии с таблицей [Отправка других типов исследований](#отправка-других-типов-исследований) . Если параметр не указан или не заполнен – считает значение 1. |
...
Алгоритм применения данных, полученных по протоколу интеграции, можно увидеть по ссылке: [Видео на яндексе](https://yadi.sk/i/eMPiG5D1m32Twg)
